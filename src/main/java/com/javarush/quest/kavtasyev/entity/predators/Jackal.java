...
@PredatorProperties(damage = 30)
public class Jackal implements Predator
{
	...
	public void attack(User user, Location location)
	{
		...
		int power = 0;
		if (arm != null)
			power = arm.getArmProperties().power();
		...
		{
			message = YOU_HAVE_FLUSHED_OFF_A_PREDATOR;
			user.shoot((FlareGun) arm);
		}
		else if (arm instanceof Machete || arm instanceof Spear || arm instanceof Truncheon)
		{
			message = String.format(YOU_HAVE_FOUGHT_OFF_A_PREDATOR, arm.getNames()[4]);
		}
		else
		{
			htmlActionButtons.delete(0, htmlActionButtons.length());
			message = YOU_HAVE_BEEN_ATTACKED_BY_A_PREDATOR;
		}
