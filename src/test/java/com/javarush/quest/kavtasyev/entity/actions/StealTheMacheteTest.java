...
class StealTheMacheteTest
{
	...
	void executeAction(double randomValue)
	{
		...
		Action action = new StealTheMachete();
		action.setRandom(random);

		String res;

		if (randomValue < locationProperties.stealMacheteProbability())
		{
			res = "<div class=\"w3-panel w3-display-container w3-leftbar w3-pale-blue w3-border-blue w3-animate-opacity\"><p>Тебе удалось стащить неаккуратно оставленный мачете.</p><span onclick=\"this.parentElement.style.display='none'\" class=\"w3-button w3-display-topright w3-hover-blue\">×</span></div>";
			assertAll(() -> {
				assertFalse(user.isHasTheArm(Machete.class));
